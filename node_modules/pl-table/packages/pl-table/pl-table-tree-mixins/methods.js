...
export default {
        ...
        if (rows) {
            ...
            if (rows.length) {
                rows.forEach(row => {
                    ...
                    // 当前row在数据中的索引值(非常重要)
                    const index = plTreeData.findIndex(item => item[this.rowKey] === row[this.rowKey])
                    // 获取当前节点下面的展开的子节点
                    ...
                    if (index >= 0) {
                        ...
                        if (expanded) {
                            ...
                            // 删除展开节点数组的row
                            XEUtils.remove(this.treeExpandeds, item => item[this.rowKey] === row[this.rowKey])
                        }
                    }
                })
                // 载入数据
    ...
    setPlTreeExpansion (rows, expanded) {
        ...
        let row = this.plTreeData.filter(item => item[this.rowKey] === rows[this.rowKey])[0]
